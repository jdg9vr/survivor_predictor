...
def get_test_ranks(X_test, y_test, prediction_prob, one_hot=True):
    if one_hot:
        winner_prob = [i[1] for i in prediction_prob]
...
hyperfast_ranks = get_test_ranks(X_test, y_test, predictions)
...
log_reg_ranks = get_test_ranks(X_test, y_test, log_reg_prob)
...
knn_ranks = get_test_ranks(X_test, y_test, knn_preds)
...
linear_svm_ranks = get_test_ranks(X_test, y_test, linear_svm_preds)
...
poly3_svm_ranks = get_test_ranks(X_test, y_test, poly3_svm_preds)
...
X_train_mini_scaled = pd.DataFrame(X_train_scaled, columns=X_train.columns)[X_train["season_no"].reset_index(drop=True).isin(train_mini_seasons)].to_numpy()
X_val_scaled = pd.DataFrame(X_train_scaled, columns=X_train.columns)[~X_train["season_no"].reset_index(drop=True).isin(train_mini_seasons)].to_numpy()
...
history = model.fit(
    X_train_mini_scaled, y_train_mini, batch_size=32, epochs=50, validation_data=(X_val_scaled, y_val)
)
...
nn_ranks = get_test_ranks(X_test, y_test, nn_preds)


def ensemble_model_ranks(X_test, y_test, **kwargs):
    count = 0
    full_test = pd.concat([X_test, y_test], axis=1)
    for model, preds_weight in kwargs.items():
        print(model)
        winner_prob = [i[1] for i in preds_weight["preds"]]
        winner_prob = pd.Series(winner_prob, name=model)

        winner_prob.index = y_test.index

        full_test = pd.concat([full_test, winner_prob], axis=1)
        full_test[model] = full_test[model]/full_test[model].groupby(full_test["season_no"]).transform("sum")

        if count == 0:
            full_test["final"] = full_test[model]*preds_weight["weight"]
        else:
            full_test["final"] += full_test[model]*preds_weight["weight"]
        count += 1


    return full_test["final"]

ensemble_preds = ensemble_model_ranks(X_test, y_test, 
                     svm={"preds":linear_svm_preds, "weight":0.5},
                     log_reg={"preds":log_reg_prob, "weight":0.2},
                     nn={"preds":nn_preds, "weight":0.3})

get_test_ranks(X_test, y_test, ensemble_preds, one_hot=False)